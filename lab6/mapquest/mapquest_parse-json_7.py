...
while 1:
    ...
    if status == 0:
        ...
        print("*"*62)
        print(f'Status Code: {status} ; Invalid user inputs for one or both locations.')
    elif status == 611:
        print("*"*62)
        print(f'Status Code: {status} ; Missing an entry for one or both locations.')
    else:
        print("*"*62)
        print(f'For Status Code: {status}; Refer to:')
